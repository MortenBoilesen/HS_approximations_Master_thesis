...
b_functions = ['flat', 'sinusoidal', 'step', 'linear', 'exp', 'logical']
...
for b_function_index, b_function in enumerate(b_functions):
    ...
    for i in range(num_experiments):
        np.random.seed(seed_0*(b_function_index + 1) + i)
        # Generate data
        x_train = np.random.uniform(0, 10, num_train)
        x_test = np.random.uniform(0, 10, num_test)
        y_train = benchmark_functions(b_function_index, x_train) + np.random.normal(0, sigma_D, x_train.shape)
        y_test = benchmark_functions(b_function_index, x_test) + np.random.normal(0, sigma_D, x_test.shape)
