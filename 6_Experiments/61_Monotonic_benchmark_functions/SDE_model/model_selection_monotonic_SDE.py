...
from utils import compute_RMSE, compute_lpd, benchmark_functions

def run_simulation(M, T, kernel):
    ...
    for i in range(args.num_experiments):
        ...
        print(f'RMSE for run {i} is: {RMSE_array[i]}')
...
if __name__ == "__main__":
    ...
    with open(f'{resultpath}model_selection_results.txt', 'w') as file:
        file.write(f"model: {benchmark_function}\n")

    for M in M_list:
        for T in T_list:
            for kernel in kernel_list:
                print(f'M={M}, T={T}, kernel={kernel}')
                print(f'M = {M} and T = {T} and kernel = {kernel}')
                result = run_simulation(M, T, kernel)
                print(f'Experiment done for M = {M} and T = {T} and kernel = {kernel}')
                RMSE_mean, RMSE_std, lpd_mean, lpd_std, RMSE_array, lpd_array = result
                with open(f'{path}results/{benchmark_function}/model_selection_results.txt', 'a') as file:
    ...
    with open(f'{path}results/{benchmark_function}/model_selection_results.pkl', 'wb') as file:
