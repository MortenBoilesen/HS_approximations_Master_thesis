...
if __name__ == "__main__":
    ...
    parser.add_argument('--m', type=int, default=1, help='number of basis functions (default: %(default)s)')
    parser.add_argument('--L', type=int, default=1, help='length of domain (default: %(default)s)')
    args = parser.parse_args()

    print(args.b_function_index)
    print(args.num_samples)
    print(args.num_warmup)
    ...
    try:
        best_m = np.load(samplepath + f'best_m.npy')
        best_L = np.load(samplepath + f'best_L.npy')
        best_RMSE = np.load(samplepath + f'best_RMSE.npy')
    except:
        best_m = 100
        best_L = 100
        best_RMSE = 1000
    ...
    if RMSE_mean < best_RMSE:
        np.save(samplepath + f'best_RMSE.npy', RMSE_mean)
        np.save(samplepath + f'best_m.npy', m)
        np.save(samplepath + f'best_L.npy', L)
