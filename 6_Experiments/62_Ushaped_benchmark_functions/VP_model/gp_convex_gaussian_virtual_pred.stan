functions {
    ...
    matrix d2x2_x_cov_exp_quad(array[] real xvirtual, array[] real x, real kappa, real scale, matrix pair_diff) {
        return ( pair_diff.*pair_diff./ scale^4 - 1/scale^2 ) .* gp_exp_quad_cov(xvirtual, x, kappa, scale);
    }
    matrix cov_exp_quad_full(array[] real x, array[] real xvirtual, int N, int num_virtual, real kappa, real scale, matrix dist_xvirtual, matrix pair_diff) {
        ...
        matrix[num_virtual, N] Sigma_d2x2_d = d2x2_x_cov_exp_quad(xvirtual, x, kappa, scale, pair_diff);
        matrix[num_virtual, num_virtual] dist_xvirtual_squared = dist_xvirtual .* dist_xvirtual;

        Sigma[1:N,1:N] = gp_exp_quad_cov(x, kappa, scale);
        Sigma[N+1:N+num_virtual, 1:N] = Sigma_d2x2_d;
        Sigma[1:N, N+1:N+num_virtual] = Sigma_d2x2_d';

        Sigma[N+1:N+num_virtual, N+1:N+num_virtual] = gp_exp_quad_cov(xvirtual, kappa, scale) .* 1./scale^4 .* ( dist_xvirtual_squared .* dist_xvirtual_squared ./ scale^4 - 6 .* dist_xvirtual_squared ./ scale^2 + 3);
    ...
    vector gp_pred_rng(array[] real x2,
  ...
  
      {
        ...
        k_x1_x2[num_train+1:num_train+num_virtual, :] = d2x2_x_cov_exp_quad(xvirtual, x2, kappa, rho, pair_diff);
...
f_pred = gp_pred_rng(xpred, f_train, xtrain, xvirtual, kappa, scale,  jitter, num_train,  num_virtual, num_pred, dist_xvirtual, diff_virtual_train);
