functions {
    ...
    vector PHI(array[] real x, real j, real L) {
        real sqrt_lambda = lambda_sqrt(j, L);
        return (to_vector(x) + L)./sqrt(L)./sqrt_lambda - sin(sqrt_lambda*(to_vector(x) + L))./sqrt(L)./(sqrt_lambda^2);
    }
...
generated quantities {
    ...
    f_pred_d2 = rep_vector(0, num_pred);
    ...
    for (j in 1:num_basis_functions) {
        ...
        for (i in 1:num_basis_functions ) {
            ...
            f_pred_d2 += (alpha_scaled[j]*phi_pred[j]).*(alpha_scaled[i]*phi_pred[i]);
        }
    }
